...
for unrolls in range(2):
	for copy in range(200000):
	
	    # Create file
		f  = open(new_dir + '/'+ 'fact2-copies' + str(copy) + '-unrolls' + str(unrolls) + '.c','w+')
		sys.stdout = f
		...
		if random.randint(0,1) == 0:
			print(init_string.format(var_i, var_i, random_no_except_n(0,10000,1))) 
		else:
			print(init_string_inline.format(var_i, random_no_except_n(0,10000,1))) 

		# Choose from one of the two possible initializations 
		if random.randint(0,1) == 0:
			print(init_string.format(var_fact, var_fact, random.randint(0, 10000)))
		else:
			print(init_string_inline.format(var_fact, random.randint(0, 10000))) 
