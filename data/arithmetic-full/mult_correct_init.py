...
for unrolls in range(5):
	for copy in range(30):
		...
		# Create file
		f  = open(new_dir + '/'+ 'mult_correct-copies' + str(copy) + '-unrolls' + str(unrolls) + '.c','w+')
		sys.stdout = f
		...
		if random.randint(0,1) == 0:
			print(init_string.format(var_i, var_i, random.randint(1,1000))) 
		else:
			print(init_string_inline.format(var_i, random.randint(1, 1000))) 

		# Choose from one of the two possible initializations 
		if random.randint(0,1) == 0:
			print(init_string.format(var_fact, var_fact, random.randint(0, 100))) 
		else:
			print(init_string_inline.format(var_fact, random.randint(0, 100))) 
		...
		for i in range(unrolls):
			...
			if random.randint(0,1) == 0:
				print(add_indent(i+1, mult_string.format(var_i, var_i, var_i)))
			else:
				print(add_indent(i+1, mult_alt.format(var_i, var_i)))
		...
		if random.randint(0,1) == 0:
			print(add_indent(unrolls+1, mult_string.format(var_i, var_i, var_i)))
		else:
			print(add_indent(unrolls+1, mult_alt.format(var_i, var_i)))
		if random.randint(0,1) == 0:
